...
def readVideo():
    ...
    inpImage = cv2.VideoCapture(r'C:\Users\rrakh\Downloads\road5.mp4')
...
def processImage(inpImage):
    ...
    lines = cv2.HoughLinesP(canny, 1, np.pi/180, threshold=80, minLineLength=400, maxLineGap=250)
    ...
    return inpImage, hls_result, gray, thresh, blur, canny, lines
...
def perspectiveWarp(inpImage):
    ...
    # src = np.float32([[570, 550],
    #                   [690, 550],
    #                   [400, 700],
    #                   [1000, 700]])

    src = np.float32([[580, 550], #Top left
                      [750, 550], #Top right
                      [410, 700], #Bottom left
...
def addText(img, radius, direction, deviation, devDirection):
    ...
    cv2.putText(img, text , (30,70), font, 0.8, (0,100, 200), 2, cv2.LINE_AA)
    cv2.putText(img, text1, (30,120), font, 0.8, (0,100, 200), 2, cv2.LINE_AA)

    # Deviation
    deviation_text = 'Off Center: ' + str(round(abs(deviation), 3)) + 'm' + ' to the ' + devDirection
    cv2.putText(img, deviation_text, (30, 170), cv2.FONT_HERSHEY_TRIPLEX, 0.8, (0,100, 200), 2, cv2.LINE_AA)
...
while True:
    ...
    img, hls, grayscale, thresh, blur, canny, lines = processImage(birdView)
    imgL, hlsL, grayscaleL, threshL, blurL, cannyL, linesL = processImage(birdViewL)
    imgR, hlsR, grayscaleR, threshR, blurR, cannyR, linesR = processImage(birdViewR)
    ...
    if lines is not None:
        for line in lines:
            x1, y1, x2, y2 = line[0]
            cv2.line(color_warp, (x1, y1), (x2, y2), (0, 0, 255), 25)

        new_warp = cv2.warpPerspective(color_warp, minverse, (width, height))
        result = cv2.addWeighted(finalImg, 1, new_warp, 0.99, 0)

    cv2.imshow("Hough Lines on Image", result)

    # # Assuming 'lines' contains detected lines in the format (x1, y1, x2, y2)
    # if lines is not None:
    #     for line in lines:
    #         x1, y1, x2, y2 = line[0]
    #         cv2.line(birdView, (x1, y1), (x2, y2), (0, 255, 0), 2)
    #
    # # Displaying final image
    # cv2.imshow("Lane Detection", finalImg)
    # cv2.imshow("L", birdView)
